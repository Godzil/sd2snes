...
)
{
    ...
    res = dir_sdi( dj, ( WORD )( ( dj->lfn_idx == 0xFFFF ) ? i :
                                 dj->lfn_idx ) ); /* Goto the SFN or top of the LFN entries */
...
)
{
    ...
    if ( btr > 512 && !ff_sd_offload )
    {
        printf( "WARNING: read >512 bytes but offloading is inactive!!\n" );
    }
            ...
            wbuff += wcnt, fp->fptr += wcnt, *bw += wcnt, btw -= wcnt )
    {
        if ( ( fp->fptr % SS( fp->fs ) ) == 0 )     /* On the sector boundary? */
        {
            ...
            if ( fp->fs->winsect == fp->dsect
                    && move_window( fp->fs, 0 ) ) /* Write back data buffer prior to following direct transfer */
            {
                ABORT( fp->fs, FR_DISK_ERR );
            }
